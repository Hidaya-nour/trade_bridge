...
export const useInventoryMovementStore = create<InventoryMovementState>((set, get) => ({
  items: [],
  currentItem: null,
  isLoading: false,
  error: null,
  ...
  fetchAll: async (params?: any) => {
    ...
    try {
      const response = await inventoryMovementService.getAll(params);
  ...
  fetchById: async (id: string) => {
    set({ isLoading: true, error: null });
    try {
      const response = await inventoryMovementService.getById(id);
  ...
  create: async (data: any) => {
    set({ isLoading: true, error: null });
    try {
      const response = await inventoryMovementService.create(data);
  ...
  update: async (id: string, data: any) => {
    set({ isLoading: true, error: null });
    try {
      const response = await inventoryMovementService.update(id, data);
  ...
  delete: async (id: string) => {
    set({ isLoading: true, error: null });
    try {
      await inventoryMovementService.delete(id);
