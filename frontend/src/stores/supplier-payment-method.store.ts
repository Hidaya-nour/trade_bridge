...
export const useSupplierPaymentMethodStore = create<SupplierPaymentMethodState>((set, get) => ({
  items: [],
  currentItem: null,
  isLoading: false,
  error: null,
  ...
  fetchAll: async (params?: any) => {
    ...
    try {
      const response = await supplierPaymentMethodService.getAll(params);
  ...
  fetchById: async (id: string) => {
    set({ isLoading: true, error: null });
    try {
      const response = await supplierPaymentMethodService.getById(id);
  ...
  delete: async (id: string) => {
    set({ isLoading: true, error: null });
    try {
      await supplierPaymentMethodService.delete(id);
