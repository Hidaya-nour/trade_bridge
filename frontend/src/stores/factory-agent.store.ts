...
export const useFactoryAgentStore = create<FactoryAgentState>((set, get) => ({
  items: [],
  currentItem: null,
  isLoading: false,
  error: null,
  ...
  fetchAll: async (params?: any) => {
    ...
    try {
      const response = await factoryAgentService.getAll(params);
  ...
  fetchById: async (id: string) => {
    set({ isLoading: true, error: null });
    try {
      const response = await factoryAgentService.getById(id);
  ...
  create: async (data: any) => {
    set({ isLoading: true, error: null });
    try {
      const response = await factoryAgentService.create(data);
  ...
  update: async (id: string, data: any) => {
    set({ isLoading: true, error: null });
    try {
      const response = await factoryAgentService.update(id, data);
  ...
  delete: async (id: string) => {
    set({ isLoading: true, error: null });
    try {
      await factoryAgentService.delete(id);
