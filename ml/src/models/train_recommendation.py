...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--models-dir",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "models",  # Fixed: parents[2]
    )
    parser.add_argument(
        "--metrics-file",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "data" / "processed" / "supplier_model_metrics.json",  # Fixed: parents[2]
    )
